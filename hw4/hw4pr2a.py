...
def grad_logreg(X, y, W, reg=0.0):
	...
	"""
	# Calculate the gradient of logistic regression with respect to W
	# gradient = X.T(sigmoid(XW) - y)+ \lambda W
	"*** YOUR CODE HERE ***"
...
def NLL(X, y, W, reg=0.0):
	...
	"""
	# Calculate the negative log likelihood of logistic regression
	"*** YOUR CODE HERE ***"
	mu = sigmoid(X @ W) 
	inSum = np.multiply(y, np.log(mu)) + np.multiply((1. - y), np.log(1. - mu))
	nll = -sum(inSum) + reg / 2 * np.linalg.norm(W) ** 2
	"*** END YOUR CODE HERE ***"
	return nll.item(0)
...
def newton_step(X, y, W, reg=0.0):
	...
	"""
	#Calculate the change of the weight according Newton's methods
	"*** YOUR CODE HERE ***"
	mu = sigmoid(X @ W)
...
def get_description(X, y, W):
	...
	"""
	# Calculate the accuracy, precision, recall, and f-1 score of the prediction
	# Compare accurate clss and predict class 
	# True Positives (TP), T Negative (TN), Flase Positive(FP), FN
	# Accuracy = TP+TN/TP+FP+FN+TN
	# Precision = TP/TP+FP = TP/ total P
	# Recall = TP/TP+FN = TP/ total actual 
	# F1 Score = 2*(Recall * Precision) / (Recall + Precision)
	"*** YOUR CODE HERE ***"
	y_pred = predict(X,W)
	m = X.shape[0] 
	count_a, count_p, count_r, total_p, total_a = 0, 0, 0, 0, 0
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
    # generate/create a list of different lambda
	reg_list = [0., 0.01, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 30.0]
	# For each lambda, run gradient descent and obtain the optimal weights
	for index in range(len(reg_list)):
		reg = reg_list[index] 
		W, n = grad_descent(X_train, y_train, reg = reg, lr = 2e-4, print_freq = 100)
	...
	"*** YOUR CODE HERE ***"
	lamb = np.argmax(a_list)
	reg_opt = reg_list[lamb]
	"*** END YOUR CODE HERE ***"
	return reg_opt
