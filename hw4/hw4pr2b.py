...
def NLL(X, y, W, reg=0.0):
	...
	"""
	# Calculate the negative log likelihood for softmax regression
	"*** YOUR CODE HERE ***"
	mu = X @ W
	e_mu = np.exp(mu)
	prob = e_mu / e_mu.sum(axis=1).reshape(-1, 1)
	inSum = y * np.log(prob)
	NLL = -inSum.sum(axis=1).sum() + reg * np.diag(W.T @ W).sum()
	"*** END YOUR CODE HERE ***"
	return NLL
...
def grad_softmax(X, y, W, reg=0.0):
	...
	"""
	# Calculate the gradient of softmax regression with respect to W
	"*** YOUR CODE HERE ***"
	mu = X @ W
	e_mu = np.exp(mu)
	prob = e_mu / e_mu.sum(axis=1).reshape(-1, 1)
...
def grad_descent(X, y, reg=0.0, lr=1e-5, eps=1e-6, max_iter=500, print_freq=20):
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		# test element-wise for NaN in obtained nll. 
		# If isnan,break out of the while loop
		if np.isnan(nll):
			break
		# append the nll to the nll_list
		nll_list.append(nll)
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
	...
	for reg in lambda_list:
		...
		# Predict the lable using weights
	...
	"*** YOUR CODE HERE ***"
	lamb = np.argmax(accu_list)
	reg_opt = lambda_list[lamb]
	"*** END YOUR CODE HERE ***"
	return reg_opt
