...
def find_RMSE(W, X, y):
	...
	"*** YOUR CODE HERE ***"
	# calculate the predicted label y_pred
	y_pred = predict(W, X)

	# Solve for the root mean-squared error, RMSE
	diff = y - y_pred
	RMSE = np.sqrt(np.linalg.norm(diff, 2) ** 2 / X.shape[0])
	"*** END YOUR CODE HERE ***"
	return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	"*** YOUR CODE HERE ***"
	# randomly generate 150 parameter settings 
	reg_list = np.random.uniform(0.0, 150.0, 150)
	# have to sort it before feeding into the function
	reg_list.sort()
	W_list = [linreg(X_train, y_train, reg = ps) for ps in reg_list]
...
def linreg_no_bias(X, y, reg=0.0):
	...
	# Identity matrix
	identm = np.eye(n) 
	# Calculate (I - 1/n 1 1^T )
	matrix = X.T @ (identm - np.ones(n) / n)
	W_opt = np.linalg.solve(matrix @ X + reg * np.eye(matrix.shape[0]), \
		matrix @ y)
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	# W (weights), W_grad (gradients of W),
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		"*** YOUR CODE HERE ***"
		# Calculate the training RMSE and validation RMSE and append 
		rmse_train = np.sqrt(np.linalg.norm((X_train @ W).reshape((-1, 1)) \
			+ b - y_train) ** 2 / m_train)
		obj_train.append(rmse_train)
		rmse_val = np.sqrt(np.linalg.norm((X_val @ W).reshape((-1, 1)) \
			+ b - y_val) ** 2 / m_val)
	...
	rmse_train_plot, = plt.plot(range(iter_num), obj_train)
	plt.setp(rmse_train_plot, color='red')
	rmse_val_plot, = plt.plot(range(iter_num), obj_val)
	plt.setp(rmse_val_plot, color='green')
	plt.legend((rmse_train_plot, rmse_val_plot), \
...
if __name__ == '__main__':
	...
	rmse_val = find_RMSE(W_with_b_1, X_val, y_val)
